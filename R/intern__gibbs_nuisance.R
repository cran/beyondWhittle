                           ...
                           full_lik=NULL) # full_lik only in use for NP 
{
  ...
  if (kmax < Inf) {
    db.list <- dbList(n, kmax, bernstein_l, bernstein_r, bernstein_coars)
  }
  ...
  for (i in 1:(Ntotal-1)) {
    ...
    if (has_NA) {
      ...
      sigma2_NA_prop <- 2 / pi * mean(f_current)
