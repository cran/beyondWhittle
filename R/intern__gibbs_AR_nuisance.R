...
gibbs_AR_nuisance_intern <- function(data, mcmc_params, prior_params, model_params, full_lik=F) {
  ...
  tol <- 1e-4
  if (excludeBoundary && abs(mean(data,na.rm=T)) > tol) {
    data <- data - mean(data)
  ...
  #sigma2_NA_prop <- var(data, na.rm=T)
  missingValues_trace[1,] <- rep(0, num_NA) #sqrt(sigma2_NA_prop) * rt(num_NA, 4)
  ...
  sigma2_trace[1] <- ar(data, order.max=ar.order, aic=F)$var.pred
  rho_trace[,1] <- ARMAacf(ar=ar(data, order.max=ar.order, aic=F)$ar, pacf=T) # Note
  ...
  for (j in 2:Ntotal) {
    ...
    if (j==2) {
      ...
      a <- pacf2AR(rho_trace[,j-1])[ar.order,]
    ...
    if ((j < Nadaptive) && (j > 1) && (j %% adaption.batchSize == 1)) {
